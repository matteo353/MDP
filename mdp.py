...
def value_iteration(mdp, gamma, epsilon):
    ...
    utilities = {}  # initialize utility dictionary
    states = mdp.get_states()  # get a list of the states
    for state in states:  # loop through states
        utilities[state] = mdp.get_reward(state)  # initialise them to their reward values

    # loop through states again updating
    iteration = 0
    while True:
        policy = derive_policy(mdp, utilities)
        print("iteration: ", iteration, "policy: ", policy)
        ...
        # loop through each state in our current iteration
        for state in states:
            temp = {}  # temporary dict for all actions
            actions = mdp.get_actions(state)  # get all possible actions from current state

            for action in actions:  # loop through possible actions
                probs = mdp.get_successor_probs(state, action)  # dictionary mapping successors to their probability
                temp[action] = mdp.get_reward(state) + gamma * (sum(probs[x] * previous[x] for x in probs))

            # Set new utility values
            utilities[state] = max(temp.values())

        # if statement to break out of our while loop - converged
        if all(abs(previous[x] - utilities[x]) < epsilon for x in states):
            #print("iteration: ", iteration, '\n', "utility: ", utilities)
            return utilities
